    ...
    <template v-else>
      ...
      <div class="content smil-row">
        <div class="col-lg-5 alat-info">
          <table class="table table-borderless">
            ...
            <tr>
              <th>Jumlah Unit Alat Terdaftar</th>
              <td>
                {{
      ...
      <div class="table-detail-content table-responsive-sm">
        <div class="table-title d-flex align-items-center">
          <h4>List Unit Alat Terdaftar</h4>
          <button
                  ...
                  <template v-if="indexContent === rows.length - 1">
                    ...
                    >
                      ...
                      <b-dropdown-item @click="deleteNotif('detail', indexRow)">
                        <span class="smil-text-danger"> Hapus Unit </span>
                      </b-dropdown-item>
                    </b-dropdown>
                  </template>
...
export default {
  ...
  data() {
    return {
      ...
      headsTable: [
        {
          id: 1,
          label: "QR Code Key Unit",
          filter_type: "search",
          placeholder: "Filter QR Code Key Unit",
          model: "barcode_alat",
          options: null,
        },
        {
          ...
          label: "Kondisi Unit",
          filter_type: "select",
          placeholder: "Filter Kondisi Unit",
  ...
  methods: {
      ...
      try {
        ...
        if (response.data.response.code == 200) {
          if (lokasi.length > 0) {
            ...
            alert("Tidak ada lokasi yang dapat menampung jumlah alat tersebut");
          }
        }
      } catch (e) {
        let message = this.getErrorMessage(e);
    ...
    async setPageAlatDetail() {
      ...
      if (this.alatDetail.habis_pakai) {
        this.formChangeCondition[1].options = optionsCondition;
      } else {
        this.formChangeCondition[1].options = optionsCondition.filter(
          (condition) => condition.name !== "Habis"
        );
      }
